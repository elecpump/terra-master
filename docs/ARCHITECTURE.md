...
2026-09-11：0.4.3 已补齐后台控制路径，前后台短帧/reset 与 30 分钟运行连续性均有实测证据。当前仍为 realtime，死亡/场景重置和 Gym/训练管线尚未实现；D1 已完成静态循环勘察，尚无同步实机结论。见 [长测报告](m0-stability-report.md)。以下历史记录保留对应阶段边界。
