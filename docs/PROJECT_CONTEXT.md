...
下一入口：验收开关作用及手动暂停，完成 M0 的 30 分钟前后台稳定性，并解决自然世界反复死亡对测试的干扰，然后以 2 天时间盒跑同步推进实验（D1），用其结论重排协议 v2 投入。不要仅因已有五元组接口就开始长时间训练。详细任务级计划见 [EXECUTION_PLAN.md](EXECUTION_PLAN.md)。
