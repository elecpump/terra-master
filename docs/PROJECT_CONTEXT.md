...
2026-09-11：M0 30 分钟前后台运行连续性通过，后台约 950 秒、前台约 848 秒，平均 59.998 tick/s，峰值工作集约 913 MiB。死亡状态样本 660 个，单独保留，不判训练就绪。全套 28 项测试通过，见 [长测报告](m0-stability-report.md)。长测期间只读勘察了安装版循环，未修改运行游戏，见 [D1 准备记录](d1-loop-investigation.md)。

下一入口：以 2 天时间盒开展 D1 同步推进实机实验；自然世界死亡干扰、安全场景与合法死亡 transition 仍须在环境切片解决。不要仅因已有五元组接口或运行稳定就开始长时间训练。详细任务级计划见 [EXECUTION_PLAN.md](EXECUTION_PLAN.md)。
