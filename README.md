...
## M0 30 分钟稳定性（2026-09-11，通过）

0.4.3 同一实例连续采样 1800 秒：后台约 15 分 50 秒、前台约 14 分 8 秒，平均 59.998 tick/s，无停滞、会话变化或陈旧快照；峰值工作集约 913 MiB。660 个死亡状态样本完整保留，运行连续性通过不代表训练场景就绪。新增进度输出和独立长测判定工具，全套 28 项离线测试通过。详见 [长测报告](docs/m0-stability-report.md)。下一阶段进入 D1 同步推进实验，当前仍是实时模式。

## 0.4.3 后台输入修复（实机验收通过）

本轮确认后台世界推进并不等于输入生效：原版失焦时跳过 SetControls。0.4.3 补充受训练门禁保护的后台桥接输入，并使用 Windows 焦点判断后台开关。26 项离线测试、安装版编译打包、前台短帧/reset 通过；0.4.3 后台开关对照和实际右移 1/6/15 帧及恢复也已通过，关闭了后台 step 零帧超时缺口。30 分钟稳定性、其他动作和无死亡训练场景仍未验收。详见 [本轮修复记录](docs/runtime-fixes-v043.md)。
